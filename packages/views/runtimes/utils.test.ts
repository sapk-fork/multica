...
describe("generated MODEL_PRICING — multi-provider coverage", () => {
  it("matches provider-prefixed models emitted by OpenCode", () => {
    for (const m of [
  ...
  it("matches date-suffixed provider-prefixed models", () => {
    expect(
      isModelPriced("anthropic/claude-sonnet-4-5-20250929"),
    ).toBe(true);
    expect(
      isModelPriced("openai/gpt-4o-2024-08-06"),
    ).toBe(true);
  });

  it("disambiguates the gpt-4o family correctly", () => {
    // Each sibling must resolve to its own price (not inherit via startsWith).
    expect(
      estimateCost({ ...zeroUsage, model: "gpt-4o", input_tokens: 1_000_000 }),
    ).toBeCloseTo(2.5, 5);
    expect(
      estimateCost({ ...zeroUsage, model: "gpt-4o-mini", input_tokens: 1_000_000 }),
    ).toBeCloseTo(0.15, 5);
    expect(
      estimateCost({ ...zeroUsage, model: "gpt-4o-2024-05-13", input_tokens: 1_000_000 }),
    ).toBeCloseTo(5, 5);
  });
});
