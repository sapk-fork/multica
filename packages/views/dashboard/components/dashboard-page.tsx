...
function Leaderboard({
  ...
  const caption =
    scope === "agent"
      ? // The deleted-agents bucket is one of agentRows, so subtract it from the
        // count and surface it via the dedicated caption (see MUL-3776).
        deletedAgentCount > 0
...
function AgentLeaderboardRow({
  ...
  const { t } = useT("usage");
  // The deleted-agents bucket is a synthetic row, not a real agent: render a
  // neutral placeholder (no avatar fetch / hover card / UUID) and dash out
  // Time/Tasks, which it never carries (see bucketUnknownAgentRows, MUL-3776).
