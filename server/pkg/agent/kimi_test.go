...
func TestKimiBackendHealthyEmptyCompletionNotFlagged(t *testing.T) {
	...
	// failure entries in the log. scanKimiSessionUsage requires the
	// record's own timestamp to be >= the backend's Execute() startTime
	// (captured moments after this seed write, with no grace period), so
	// the record is post-dated comfortably ahead of "now" to avoid racing
	// that internal clock read.
	seedKimiSessionFiles(t, home, "ses_swallowed",
		kimiWireRecordLineFor("kimi-code/k3", now.Add(time.Minute), 100, 10, 0, 0)+"\n",
