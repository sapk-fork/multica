...
| **Kimi** | Moonshot | ✅ | ✅ | `$KIMI_CODE_HOME/skills/`（タスクごとの home） | 動的探索 |
...
Moonshot が提供し、中国市場を対象としています。Hermes と ACP プロトコルを共有し、MCP 構成も ACP `mcpServers` として渡されます。スキルはタスクごとの `$KIMI_CODE_HOME/skills/`（Kimi Code CLI 自身の User ティアのスキャン位置）に展開されるため、ユーザーの実際の `~/.kimi-code/` に触れることなくネイティブに探索されます。
...
| Kimi | `$KIMI_CODE_HOME/skills/`（タスクごとの home） | ✅ ネイティブ（Kimi Code CLI の User ティアのスキャン位置） |
