...
| **Kimi** | Moonshot | ✅ | ✅ | `$KIMI_CODE_HOME/skills/` (per-task home) | Dynamic discovery |
...
From Moonshot, aimed at the Chinese market. Shares the ACP protocol with Hermes, including MCP config through ACP `mcpServers`. Skills are hydrated into a per-task `$KIMI_CODE_HOME/skills/` — Kimi Code CLI's own User-tier scan location — so discovery is native without touching the user's real `~/.kimi-code/`.
...
| Kimi | `$KIMI_CODE_HOME/skills/` (per-task home) | ✅ Native (Kimi Code CLI's User-tier scan location) |
