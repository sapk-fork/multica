...
| **Kimi** | Moonshot | ✅ | ✅ | `$KIMI_CODE_HOME/skills/`（按任务隔离的 home） | 动态发现 |
...
Moonshot 出品，中国市场向。和 Hermes 共享 ACP 协议，MCP 配置同样通过 ACP `mcpServers` 传入。skills 会被注入按任务隔离的 `$KIMI_CODE_HOME/skills/`——这是 Kimi Code CLI 自身的 User 级扫描位置——因此发现机制是原生的，且不会改动用户真实的 `~/.kimi-code/`。
...
| Kimi | `$KIMI_CODE_HOME/skills/`（按任务隔离的 home） | ✅ 原生（Kimi Code CLI 的 User 级扫描位置） |
