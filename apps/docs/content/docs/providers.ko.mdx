...
| **Kimi** | Moonshot | ✅ | ✅ | `$KIMI_CODE_HOME/skills/`(태스크별 home) | 동적 탐색 |
...
Moonshot에서 제공하며, 중국 시장을 겨냥합니다. Hermes와 ACP 프로토콜을 공유하고 MCP 구성도 ACP `mcpServers`로 전달됩니다. 스킬은 태스크별 `$KIMI_CODE_HOME/skills/`(Kimi Code CLI 자체의 User 티어 스캔 위치)에 주입되므로 사용자의 실제 `~/.kimi-code/`를 건드리지 않고 기본 탐색이 동작합니다.
...
| Kimi | `$KIMI_CODE_HOME/skills/`(태스크별 home) | ✅ 기본 (Kimi Code CLI의 User 티어 스캔 위치) |
