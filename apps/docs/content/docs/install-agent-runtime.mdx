...
ACP-protocol agent, primarily aimed at the Chinese market. MCP config is passed through ACP `mcpServers`; skills live under `$KIMI_CODE_HOME/skills/` in a per-task home the daemon sets up (Kimi Code CLI's User-tier scan location — native discovery).
